...
const scheduleAppointmentController = async (req, res) => {
    try {
        ...
        if (user?.email) {
            (0, emailService_1.sendEmail)(user.email, "Turno agendado", `Hola ${user.name}, tu turno fue agendado para el día ${date} a las ${time}.`).catch((err) => console.error("Error enviando email:", err));
        }
...
const cancelAppointmentController = async (req, res) => {
    try {
        ...
        if (appointmentWithUser?.user?.email) {
            (0, emailService_1.sendEmail)(appointmentWithUser.user.email, "Turno cancelado", `Hola ${appointmentWithUser.user.name}, tu turno programado para el día ${appointmentWithUser.date} a las ${appointmentWithUser.time} ha sido cancelado.`).catch((err) => console.error("Error enviando email:", err));
        }
        res.status(200).json({
